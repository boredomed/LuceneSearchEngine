...
public class Indexer {
   ...
   private void indexFile(File file) throws IOException {
      ...
      document.add(new Field("doclength", Long.toString(file.length()), Field.Store.YES, Field.Index.ANALYZED));
   ...
   public int createIndex(String dataDirPath, FileFilter filter) 
      ...
      for (File file : files) {
         if(!file.isDirectory()
            ...
            indexFile(file);
            averageDocLen+=file.length();
         }
      }
      int total_docs = writer.maxDoc();
      writer.optimize();
      writer.close();
      return total_docs;
   }
   
   public double getterAverageDocLength(){
